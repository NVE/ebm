...
class EnergyRequirement:
    ...
    def calculate_energy_requirement(self, all_building_categories, all_purpose, all_teks, erq_oc, model_years,
        ...
        erq_oc = explode_column_alias(erq_oc, 'building_category', all_building_categories, 'default')
        ...
        reduction_per_condition = explode_column_alias(reduction_per_condition,
                                                       'building_category',
                                                       all_building_categories,
                                                       'default')

        reduction_per_condition = explode_column_alias(df=reduction_per_condition, column='TEK', values=all_teks, alias='default',
        ...
        reduction_per_condition.loc[:, 'reduction_condition'] = reduction_per_condition.loc[:, 'reduction_condition'].fillna(1.0)

        condition_factor = pd.merge(left=reduction_per_condition, right=pd.DataFrame({'year': model_years}), how='cross')
        condition_factor = pd.merge(df_years, condition_factor)

        policy_improvement = database_manager.get_energy_req_policy_improvements()
        policy_improvement = explode_column_alias(policy_improvement, 'building_category', all_building_categories, 'default')
        policy_improvement = explode_column_alias(policy_improvement, 'TEK', all_teks, 'default')

        policy_improvement = pd.merge(right=pd.DataFrame({'year': model_years}), left=policy_improvement, how='cross')
        policy_improvement.loc[:, 'year_no'] = policy_improvement.loc[:, 'year'] - policy_improvement.loc[:, 'period_start_year']
        ...
        policy_improvement.loc[policy_improvement_slice, 'reduction_policy'] = policy_improvement.loc[policy_improvement_slice].apply(
            yearly_reduction, axis=1)

        y_i = database_manager.get_energy_req_yearly_improvements()
        ...
        period_end_gt_start_year = (yearly_improvements[yearly_improvements.period_end_year > yearly_improvements.efficiency_start_year]).index
        ...
        yearly_improvements.loc[:, 'reduce_efficiency_improvement'] = 1.0 - yearly_improvements.loc[:, 'yearly_efficiency_improvement']
        ...
        merged = pd.merge(left=m_nrg_yi.copy(), right=condition_factor.copy(),
