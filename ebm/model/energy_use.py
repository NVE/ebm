...
def base_load(heating_systems_parameters):
    ...
    df = df.rename(columns={GRUNNLAST_ANDEL: 'load_share',
                                  GRUNNLAST_VIRKNINGSGRAD: 'load_efficiency',
...
def peak_load(heating_systems_parameters):
    ...
    df = df.rename(columns={SPISSLAST_ANDEL: 'load_share',
                                  SPISSLAST_VIRKNINGSGRAD: 'load_efficiency',
...
def tertiary_load(heating_systems_parameters):
    ...
    df = df.rename(columns={EKSTRALAST_ANDEL: 'load_share',
                                  EKSTRALAST_VIRKNINGSGRAD: 'load_efficiency',
...
def heating_dhw(heating_systems_parameters):
    df = heating_systems_parameters[['building_category', 'TEK', 'year', 'heating_systems', TEK_SHARES, GRUNNLAST_ANDEL, DHW_EFFICIENCY, TAPPEVANN_ENERGIVARE]].copy()
    df.loc[:, GRUNNLAST_ANDEL] = 1.0
    df = df.rename(columns={GRUNNLAST_ANDEL: 'load_share',
                                DHW_EFFICIENCY: 'load_efficiency',
...
def cooling(heating_systems_parameters):
    df = heating_systems_parameters[
        ['building_category', 'TEK', 'year', 'heating_systems', TEK_SHARES,
         GRUNNLAST_ANDEL, KJOLING_VIRKNINGSGRAD, GRUNNLAST_ENERGIVARE]].copy()
    df.loc[:, GRUNNLAST_ANDEL] = 1.0
    # df_c.loc[:, KJOLING_VIRKNINGSGRAD] = 4.0
    df.loc[:, GRUNNLAST_ENERGIVARE] = 'Electricity'
    df = df.rename(columns={
        GRUNNLAST_ANDEL: 'load_share',
        KJOLING_VIRKNINGSGRAD: 'load_efficiency',
        GRUNNLAST_ENERGIVARE: 'energy_product'})
...
def all_purposes(heating_systems_parameters: pd.DataFrame) -> pd.DataFrame:
    pd.concat(
        [heating_rv(heating_systems_parameters),
         heating_dhw(heating_systems_parameters),
         cooling(heating_systems_parameters),
         other(heating_systems_parameters)])
