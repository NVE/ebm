...
def calculate_construction(building_category: BuildingCategory=None):
    ...
    kg_barnehage_nybygget = pd.Series(data=[97_574, 90_644, 65_847, 62_022, 79_992], index=[2010, 2011, 2012, 2013, 2014])
    logger.info(kg_barnehage_nybygget)

    logger.debug('kg_areal_befolkningsvekst (2040-2050)')
    kg_areal_befolkningsvekst = pd.Series(data=100, index=[y for y in range(2040, 2051)])
    logger.info(kg_areal_befolkningsvekst)
    ...
    befolkning = pd.read_csv('C:/Users/kenord/dev2/Energibruksmodell/input/nybygging_befolkning.csv', dtype={"household_size": "float64"})
    ...
    logger.debug('kg_barnehage_nybygget_2010 1 275 238,0000')
    logger.info(kg_barnehage_nybygget.loc[2010])
    ...
    logger.debug(f'{kg_totalt_areal_2010.loc[2010]=} - {kg_arlig_revet_areal.loc[2011]=} + {kg_barnehage_nybygget.loc[2011]=}')
    ...
    logger.debug(f'{kg_totalt_areal_2010.loc[2011]=} - {kg_arlig_revet_areal.loc[2012]=} + {kg_barnehage_nybygget.loc[2012]=}')
    logger.debug('sum 1 429 891')
    logger.info(kg_totalt_areal_2010)

    for year in range(2011, 2015):
        logger.debug(f'{year} { kg_totalt_areal_2010.loc[year-1]} { kg_arlig_revet_areal.loc[year]} { kg_barnehage_nybygget.loc[year] }')
        kg_totalt_areal_2010.loc[year] = kg_totalt_areal_2010.loc[year-1]-kg_arlig_revet_areal.loc[year] + kg_barnehage_nybygget.loc[year]
    ...
    kg_barnehagevekst = pd.Series(data=itertools.repeat(0.0, 41), index=[y for y in range(2010, 2051)])
    ...
    #kg_floor_area_population.loc[2010] =
    #barnehagevekst_2011 = (kg_totalt_areal_2010.loc[2011] / kg_totalt_areal_2010.loc[2010]) - 1
    #kg_barnehagevekst = pd.Series(data=[barnehagevekst_2011], index=[2011])
    kg_floor_area_population = pd.Series(data=[np.nan] + list(itertools.repeat(1, 40)), index=range(2010, 2051))
    kg_floor_area_population.loc[2011] = barnehagevekst_2011/population_growth.loc[2011]

    logger.debug('kg_floor_area_population')
    ...
    for year in range(2021, 2031):
        kg_floor_area_population.loc[year] = (kg_floor_area_population.loc[2020] - (year - 2020) * ((kg_floor_area_population.loc[2020]-kg_floor_area_population.loc[2030])/10))
    ...
    ###Årlig nybygget akkumulert
    kg_arlig_nybygget_akkumulert = kg_barnehage_nybygget.cumsum()
    logger.info(kg_arlig_nybygget_akkumulert)

    ## Barnehage nybygg rate

    kg_nybygg_rate = kg_barnehage_nybygget / kg_totalt_areal_2010
    logger.info(kg_nybygg_rate)

    return kg_barnehage_nybygget
