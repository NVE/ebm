...
from ebm.model.column_operations import explode_column_alias, replace_column_alias
...
def test_replace_column_alias():
    df = pd.DataFrame({'category': ['A', 'B', 'default'], 'extra': [0, 1, 2]})

    result = replace_column_alias(df, column='category', values=['A', 'B'], alias='default')
    assert result.category.to_list() == ['A', 'B', 'A+B']

    result = replace_column_alias(df, column='category', values=['A', 'B'])
    assert result.category.to_list() == ['A', 'B', 'A+B']

    result = replace_column_alias(df, column='category')
    assert result.category.to_list() == ['A', 'B', 'A+B']

    result = replace_column_alias(df, column='category', de_dup_by=['category'])
    assert result.category.to_list() == ['A', 'B', 'A+B']
    assert result.extra.to_list() == [0, 1, 2]

    result = replace_column_alias(df, column='category', values={'default': ['A', 'B']})
    assert result.category.to_list() == ['A', 'B', 'A+B']

    result = replace_column_alias(df=pd.DataFrame({'category': ['A', 'B', 'beta', 'alpha', 'default']}),
                                  column='category',
                                  values={'default': ['A', 'B'], 'alpha': ['a'], 'beta': ['b']})
