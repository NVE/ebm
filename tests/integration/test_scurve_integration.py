...
def test_scurves():
    ...
    s_curve_parameters = ebm.areaforecast.s_curve.translate_scurve_parameter_to_shortform(pd.read_csv(parameters_csv))
    scurve_rates = ebm.areaforecast.s_curve.scurve_rates(s_curve_parameters)
    actual = ebm.areaforecast.s_curve.scurve_rates_with_age(scurve_rates)[['rate', 'rate_acc']].query('age<=130')
